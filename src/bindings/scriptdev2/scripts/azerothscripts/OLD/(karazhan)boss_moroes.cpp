...
<<<<<<< .mine
#define SPELL_STRIKE	15667	// 1 sinister strike da 900 di danno.(+weapon)
#define SPELL_STUN	41389	// stun x 4 sec
#define SPELL_EVASION	31379	// x dodgare di +
#define SPELL_FLURRY	33735	// aumenta velocità d'attacco del 20%
#define SPELL_GSTRIKE	31022	// danno + 15% dodge x 7 sec
#define SPELL_GARROTE	37066	// 107k di danno over 5 min. lo uso su attack start
#define SPELL_CSHADOWS	39666	// x 5 sec è praticamente invulnerabile alle spell
=======
...
<<<<<<< .mine
#define SAY_AGGRO	"mmm, ospititi inattesi? bisogna accoglerli!"
#define SAY_SPECIAL	"come, non riesci nemmeno a muoverti?"
#define SAY_KILL1	"un altro per cena, sta sera"
#define SAY_KILL2	"mai abbastanza tempo per sopravvivere, eh?"
#define SAY_KILL3	"nessuna pietà!"
#define SAY_DEATH	"vergogna... che impacciato!"
=======
...
{
    boss_moroesAI(Creature *c) : ScriptedAI(c) {Reset();}
=======
...
<<<<<<< .mine
    uint32 strike_timer;
    uint32 stun_timer;
    uint32 evasion_timer;
    uint32 flurry_timer;
    uint32 gstrike_timer;
    uint32 cshadows_timer;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (stun_timer < diff)
        {
            DoCast(m_creature->getVictim(),SPELL_STUN);
            stun_timer = 30000;
        ...
        {
            DoCast(m_creature->getVictim(),SPELL_FLURRY);
            flurry_timer = 25000;
        }
        else flurry_timer -= diff;

        if(gstrike_timer < diff)
        {
            DoCast(m_creature->getVictim(),SPELL_GSTRIKE);
            gstrike_timer = 600000;
        }
        else gstrike_timer -= diff;


        if(cshadows_timer < diff)
        {
            DoCast(m_creature->getVictim(),SPELL_CSHADOWS);
            cshadows_timer = 21000;
        }
        else cshadows_timer -= diff;
