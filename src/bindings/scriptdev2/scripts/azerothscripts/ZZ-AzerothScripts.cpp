...
{	
   ...
   if(!current_summon){ // non trovato
	   ...
	   tmp = player->SummonCreature(AZEROTH_AIUTANTE, x, y, z, 4.14, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, spawntime);
...
if(player->isInCombat()) {
      _Creature->MonsterSay("Sei in combat, sconfiggi prima i tuoi nemici! || You are in combat! Fight your battle!", LANG_UNIVERSAL, NULL);
   return;
 }
...
struct sm *current_pet_trainer = dlsearch(player->GetGUID(), 2, &list_head);
	...
	case 3:
	    ...
	    switch(player->getClass())
		{
			...
			case CLASS_DEATH_KNIGHT: itemset = 60; break; //classe mancante
   ...
   if(!current_trainer){ // non trovato 
	   player->GetClosePoint(x,y,z,player->GetObjectSize());
	   tmp = player->SummonCreature(trainerid, x, y, z, 4.14, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, spawntime);
  ...
  break; 
   ...
   if(!current_pet_trainer){ // non trovato 
	   player->GetClosePoint(x,y,z,player->GetObjectSize());
	   tmp = player->SummonCreature(trainerid, x, y, z, PET_FOLLOW_ANGLE, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, spawntime);
   ...
   { //se lo ha trovato cancelliamo lo spawn dal mondo se c'e' ancora e ne aggiungiamo uno nuovo 
    ...
    case 13:  
		...
		} else _Creature->MonsterSay("Devi avere almeno 50 crediti per poterli convertire! || You must have at least 50 credits to convert them!", LANG_UNIVERSAL, NULL);  
	   return;
      break;
...
void SendPets_Menu(Player *player, Creature *_Creature, uint32 action )
{
    ...
    if (PetId == 0) return;
	  ...
	  player->GetClosePoint(x,y,z,player->GetObjectSize());
      Pet = _Creature->SummonCreature(PetId, x, y, z, PET_FOLLOW_ANGLE, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 6000);
...
// Main menu
	...
	{
		_Creature->MonsterSay("Non hai ancora il livello adatto - You don't have the level for it",LANG_UNIVERSAL,NULL);
	    return false;
	} else
	{
