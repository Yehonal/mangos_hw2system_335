...
void Hw2Class::DmPlayerInfo(Player *pl)
{
	if (!pl)
		...
		if (TrMod[2]){ stringa="on"; if (TrMod[3]) stringa2="( con raid )( raid enabled )"; else stringa2="( senza raid )( raid disabled )";}
		else { stringa="off"; stringa2=" ";}
		ChatHandler(pl).PSendSysMessage("|             [ Group ][ Gruppo ]:         %s %s ",stringa,stringa2);
		if (TrMod[4]) stringa="on"; else stringa="off";
		ChatHandler(pl).PSendSysMessage("|               [ Duel ][ Duello ]:         %s ",stringa);
        if (TrMod[5] || pl->GetZoneId()!=616 ) stringa="Deathmatch"; else stringa="Faction War"; // se non si e' sull'hyjal e' sempre deathmatch
...
{ 
	...
	if (SalvaSulDb){
           ...
           QueryResult *result = Hw2Database.PQuery("SELECT * FROM `a_tournament_punti` WHERE `guid`= %u",pl->GetGUID());
           if(!result) { Hw2Database.PQuery("INSERT INTO `a_tournament_punti` VALUES ('%u','%u','%u','%u','%d','%d')",pl->GetGUID(),pl->Uccisioni,pl->Morti,pl->Suicidi,pl->Crediti,pl->Totale);
					    return;}              
           Hw2Database.PQuery("UPDATE `a_tournament_punti` SET `uccisioni` ='%u' , `morti` ='%u' , `suicidi` = '%u' , `crediti` ='%d', `totale` ='%d' WHERE `guid`='%u'",pl->Uccisioni,pl->Morti,pl->Suicidi,pl->Crediti,pl->Totale,pl->GetGUID());
		   delete result;
	       return;
	} 
	...
	if (HonorKill)
		{
			...
			uint32 Shonor = soggetto->Totale<0 ? 0 : soggetto->Totale;;  // cambiato GetHonorPoints() con totale
			...
			pl->Crediti += temp * sWorld.getConfig(CONFIG_FLOAT_RATE_HONOR);  pl->Totale += temp*sWorld.getConfig(CONFIG_FLOAT_RATE_HONOR);
			...
			srand(time(NULL));
			switch(rand()%13){
				case 0: suono=11215; break;
				case 1: suono=11217; break;
				case 2: suono=10564; break;
				case 3: suono=10565; break;
				case 4: suono=11472; break;
				case 5: suono=10282; break;
				case 6: suono=9169; break;
				case 7: suono=11541; break;
				case 8: suono=8293; break;
				default: suono=11215; break;
			}
			...
			pl->FrenzyCount=0; pl->Frenesia=false; 		
			uint32 spellId[4]={DEMONIC_FRENZY,SPELL_DAMAGE,AURA_FRENESIA,ELECTROMEDAL};
		    for (uint8 i=0;i<4;i++)
			...
			if (!suicidio)  {
				...
				pl->Crediti -= pMorte; pl->Totale -= pMorte;
			...
			{   
				...
				pl->Crediti -= pSuicidio; pl->Totale -= pSuicidio;
			...
			Hw2Database.PQuery("UPDATE `a_tournament_punti` SET `crediti` ='%d',`totale` ='%d' WHERE `guid`=%u",pl->Crediti,pl->Totale,pl->GetGUID()); // le morti vengono salvate ogni volta <ANTI LAMER>
			srand(time(NULL));
			switch(rand()%3){
				case 0: suono=10566; break;
				case 1: suono=10284; break;
				case 2: suono=8622; break;
			}
			DmGestioneMusica(pl,suono); 
			return;
      	}
  ...
  ChatHandler(pl).AzerothQuest(pl);
   ...
   else
   {
		...
		else
		{
				...
				QueryResult *result2 = Hw2Database.PQuery("SELECT * FROM `a_tournament_punti` WHERE `guid`= %u",pl->GetGUID());
				if(result2)  
		        {
					Field *fields = result2->Fetch();
					pl->Uccisioni = fields[1].GetUInt32();  // 0 è uguale al guid
...
{	    
	...
	ChatHandler(pl).AzerothSelezionaHandler(2,pl->GetName());              // resetta tutti i talents e le relative spells
