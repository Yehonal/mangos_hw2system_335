...
int Hw2Class::dlprocess(uint16 flag, Player *pl, Creature *cr, struct act *start) {
	...
	if (rand()%100<50)  // 50% di possibilità di interagire
    	return 70;
	  ...
	  if (( Ls_act->loc_type==1 && Ls_act->loc_id==map ) || ( Ls_act->loc_type==2 && Ls_act->loc_id==zone ) || ( Ls_act->loc_type==3 && Ls_act->loc_id==area) )
		 if (rand()%100<=Ls_act->chance)
		 {
			...
			if (cr->GetDefaultMovementType()==WAYPOINT_MOTION_TYPE && rand()%100<=80)
			{
				 ...
				 cr->SendMonsterMoveWithSpeed(x,y,z,((1000)*((uint32)cr->GetDistance(pl))),pl);
				 cr->walk_timer=10000;
			}
		 ...
		 if (count<count_list)	{ Ls_act = Ls_act->next; sLog.outDebug("Avanzamento Lista");}
...
Player* Hw2Class::AzerothSelectNearbyFrTarget(Unit *unit) const
{
   if (!unit) 
...

	{
        ...
        MaNGOS::PlayerSearcher<MaNGOS::AnyPlayerInObjectRangeCheck> searcher(unit,pl, u_check);
        
		TypeContainerVisitor<MaNGOS::PlayerSearcher<MaNGOS::AnyPlayerInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        cell.Visit(p,world_unit_searcher, *unit->GetMap(),*unit, radius);
    }
...
std::list<Unit *> Hw2Class::SelectNearbyTargets(Unit* unit,uint8 tipo,float distanza) const
{
  ...
  if (!unit)
	...
	if (tipo==1)
	{
    	    ...
    	    cell.Visit(p, world_unit_searcher, *unit->GetMap(), *unit , distanza);
...
bool Hw2Class::RpgGestioneEmote(Player *pl,uint32 emote)
{
    ...
    cr=ObjectAccessor::GetCreatureInWorld(pl->GetSelection());
...
bool Hw2Class::RpgModificaPT(bool salva,uint64 guid,int32 credito,int32 totale,bool indirect_pt,Player *Modder)
{
	...
	if (salva)
	{
		...
		{ 
		   uint8 sesso=0;
		    ...
		    Hw2Database.PQuery("INSERT INTO `a_rpg_players` VALUES ('%u','%d','%d','Non definita','0','%u')",GUID_LOPART(guid),credito,credito,sesso);
		}      
		...
		if(pl)
		{
		   ...
		   if (pl->RpgSupervisor==0 && pl->RpgTotalePt>=cl[1].grado)  // se non ha il supervisore dopo il primo lvl , ritorna
		   {
				...
				if (Modder)  
					ChatHandler(Modder).PSendSysMessage("Il player indicato, ora, deve prima scegliere un precettore",guid);
				return false;
		   }
           ...
           Hw2Database.PQuery("UPDATE `a_rpg_players` SET `credito_rpg` ='%d' , `totale_rpg` ='%d' WHERE `guid`='%u'",pl->RpgCredito,pl->RpgTotalePt,guid);
		   delete result;
	...
	{  		//modalita' modifica punti senza salvataggio
	...
	if (Modder)  // nel caso sia un modder ad eseguira la procedura..gli sara' segnalato il feedback
		ChatHandler(Modder).PSendSysMessage("I punti gdr del player %u sono stati modificati",guid);
...
bool Hw2Class::RpgModIdentity(uint64 guid,char* stringa,Player *Modder)
{
	...
	{ 
	    ...
	    Hw2Database.PQuery("INSERT INTO `a_rpg_players` VALUES ('%u','0','0','%s','0','%u')",GUID_LOPART(guid),stringa,0,sesso);
	}      
	else
	{
		...
		Hw2Database.PQuery("UPDATE `a_rpg_players` SET `identity`='%s' WHERE `guid`='%u'",stringa,GUID_LOPART(guid));
		delete result;
	}

	if (Modder)
		ChatHandler(Modder).PSendSysMessage("L'identita' del player %u e' stata modificata in: %s",GUID_LOPART(guid),stringa);
...
bool Hw2Class::RpgSetSupervisor(uint64 guid, uint64 supervisor,Player *Modder,bool Force)
{
    ...
    if (guid==0 || guid==supervisor || supervisor==0) //non esegue nulla sotto queste condizioni
		return false;


	Player* pl=sObjectMgr.GetPlayer(guid);  // recupera il player dal guid

	QueryResult *rtemp = Hw2Database.PQuery("SELECT supervisor,totale_rpg FROM `a_rpg_players` WHERE `guid`= %u",GUID_LOPART(supervisor));
	if (!rtemp)
	{
		if (pl)
	...
	uint64 stemp=0;
	int32 ttemp=0;
	stemp=ftemp[0].GetUInt64();
	ttemp=ftemp[1].GetInt32();
	delete rtemp;

	if (guid==stemp || sHw2.RpgTrovaRank(ttemp)<1) // non puoi scegliere come supervisore un tuo supervisore o qualcuno di livello 0
	{
		if (pl)
			ChatHandler(pl).PSendSysMessage("Non puoi scegliere come precettore un accolito, o te stesso");
		return false;
	}

	QueryResult *result = Hw2Database.PQuery("SELECT supervisor FROM `a_rpg_players` WHERE `guid`= %u",GUID_LOPART(guid));
	if(!result) 
	{ 
		uint8 sesso=0;
	    ...
	    Hw2Database.PQuery("INSERT INTO `a_rpg_players` VALUES ('%u','0','0','Non definita','%u','%u')",GUID_LOPART(guid),GUID_LOPART(supervisor),sesso);
	}   
	else
	{
		...
		Hw2Database.PQuery("UPDATE `a_rpg_players` SET `supervisor`='%u' WHERE `guid`='%u'",GUID_LOPART(supervisor),GUID_LOPART(guid));
...
bool Hw2Class::RpgProfiloOffLine(uint64 guid,Player *Modder)
{
   if (guid==0) 
    ...
    {
		Field *fields		= result->Fetch();
		int32 RpgCredito		    = fields[1].GetInt32();  // 0 è uguale al guid
		int32 RpgTotalePt		    = fields[2].GetInt32();
		std::string RpgIdentity		= fields[3].GetCppString();
		uint64 RpgSupervisor        = fields[4].GetUInt64();
        ...
        if (RpgSupervisor!=0)
			sObjectMgr.GetPlayerNameByGUID(RpgSupervisor,name);
...
int32 Hw2Class::RpgContaDownLine(uint64 guid,uint16 pl_rango)
{
   ...
   QueryResult *result=NULL;
		...
		result = Hw2Database.PQuery("SELECT guid,totale_rpg FROM `a_rpg_players` WHERE `supervisor`= %u",GUID_LOPART(guid));
		if(!result) 
		{
...
bool Hw2Class::RpgAggiornaUpLine(uint64 guid, bool salva, float punti)
{
   ...
   do
   {
		...
		result = Hw2Database.PQuery("SELECT supervisor,totale_rpg FROM `a_rpg_players` WHERE `guid`= %u",GUID_LOPART(guid));
		if(!result) 
		{
			return true;
		} else
		{
			...
			if (punti!=0) // assegnazione punti agli upline
			{
				...
				if (punti>0)
				{
					...
					if (percentuale<=0)
					 ...
					 sHw2.RpgModificaPT(salva,guid,(int32)percentuale,0,true);
			...
			guid=fields[0].GetUInt64(); // recupera il guid del prossimo elemento
